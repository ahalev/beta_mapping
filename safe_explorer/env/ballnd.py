...
class BallND(gym.Env):
    ...
    def render(self, mode='human'):
        plt.scatter(self._agent_position[0], self._agent_position[1], label='Agent')
        plt.scatter(self._target_position[0],self._target_position[1],  label='Target')

        slack = self._config.agent_slack
        constraint_x = [slack, 1-slack, 1-slack, slack, slack]
        constraint_y = [slack, slack, 1-slack, 1-slack, slack]
        plt.plot(constraint_x, constraint_y, color='k')
        plt.legend()
        plt.show()


class BallND2(BallND):
    """
    Same as BallND but without the weird dict of observations
    """
    def __init__(self, n=None):
        super().__init__()
        self.observation_space = Box(low=0, high=1, shape=(2*self._config.n,))
        if n is not None:
            self.action_space = Box(low=-1, high=1, shape=(n,), dtype=np.float32)
            self.observation_space = Box(low=0, high=1, shape=(2*n,))
            self._config.n = n

    def step(self, action):
        # Uncomment these two lines if you are using the L2 safety layer and not the mapping:
        # action -= 0.5
        # action *= 2
